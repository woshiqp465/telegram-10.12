...
(function() {
  ...
  // Emoji 列表（按分类组织，总计300+个）
  const emojiCategories = {
    'all': {
      icon: '🌟',
      name: '全部',
      emojis: []  // 将在后面填充所有emoji
    },
    'smileys': {
      icon: '😊',
      name: '笑脸',
    ...
    const html = `
      <div id="chat-widget" style="position:fixed;bottom:20px;${posStyle};z-index:999999">
        ...
        <div id="chat-window" style="display:none;position:absolute;bottom:80px;${windowPosStyle};width:400px;height:600px;background:white;border-radius:12px;box-shadow:0 8px 24px rgba(0,0,0,0.2);flex-direction:column;overflow:hidden">
          ...
          <!-- Emoji 选择器 -->
          <div id="emoji-picker" style="display:none;max-height:250px;overflow-y:auto;background:white;border-top:1px solid #e0e0e0"></div>
  ...
  function initEmojiPicker() {
    ...
    categories.id = 'emoji-categories';
    categories.style.cssText = 'display:flex;gap:4px;padding:8px;border-bottom:1px solid #e0e0e0;overflow-x:auto;background:white;position:sticky;top:0;z-index:10';
    ...
    Object.entries(emojiCategories).forEach(([key, cat]) => {
      ...
      btn.style.cssText = `border:none;background:none;font-size:22px;padding:4px 8px;cursor:pointer;border-radius:6px;transition:all 0.2s;${key === 'all' ? 'background:#f0f0f0' : ''}`;
      btn.onmouseover = () => { if (currentEmojiCategory !== key) btn.style.background = '#f7f7f7'; };
      btn.onmouseout = () => { if (currentEmojiCategory !== key) btn.style.background = 'none'; };
    ...
    container.style.cssText = 'padding:8px;overflow-y:auto;max-height:150px';
